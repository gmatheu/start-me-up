...
if ! (which idea > /dev/null)
  ...
  if [ -f ideaIC-12.1.4.tar.gz ]; then
    ...
    sudo sh -c 'cat > /usr/share/applications/idea.desktop <<EOF
[Desktop Entry]
Name=IntelliJ Idea 12 Community
Type=Application
...
Name[en]=Idea 12 Community
EOF'
    sudo ln -s /opt/$IDEA_PREFIX/bin/idea.sh /usr/local/bin/idea
  else 
